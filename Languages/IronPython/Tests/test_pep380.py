...
class TestPEP380Operation(unittest.TestCase):
    ...
    def test_next_raises_stop_iteration(self):
		...
		a = f()
		try:
			next(a)	 # got 42, then StopIteration
    ...
    def test_send_throw_non_generator(self):
		trace = []
		class MyIt(object):
			def __init__(self):
				pass
		...
		a = f()
		self.assertEqual(next(a), 100)
		
		self.assertEqual(a.throw(Exception), 200) 
		self.assertEqual(trace, ['next', 'throw'])
		
		trace = []
		a = f()
		self.assertEqual(next(a), 100)
		try:
			a.throw(GeneratorExit)
		except GeneratorExit:
			pass
		self.assertEqual(trace, ['next', 'close'])
		
		trace = []
		a = f()
		next(a)
		self.assertEqual(a.send(42), 300)
		self.assertEqual(trace, ['next', 'send'])
		
    def test_non_generator_stop_iteration(self):
		class MyIt(object):
			def __init__(self):
				pass
			def __iter__(self):
			...
			def send(self, value):
				trace.append('send')
				raise StopIteration(300)
			def throw(self, *abc):
				trace.append('throw')
				raise StopIteration(200)
		...
		try:		
			...
			self.assertEqual(se.value, None)
		else:
			self.fail('no StopIteration')
		self.assertEqual(trace, ['next', 'throw', 200])
		...
		try:
			...
			self.fail('no StopIteration')
		self.assertEqual(trace, ['next', 'send', 300])
